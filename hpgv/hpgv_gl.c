...
typedef struct hpgv_gl_context_t {    
    ...
    void            *databuf;
    int             databuf_format; /* HPGV_RGBA or HPGV_RAF*/
    int             databuf_type;   /* HPGV_FLOAT */
...
int hpgv_gl_dbformat(int format)
{
    ...
    if (format != HPGV_RGBA && format != HPGV_RAF) {
        char error[1024];
        sprintf(error, "Unsupported format %d", format);
        HPGV_ABORT(error, 1);
    }
    
    theGLContext->databuf_format = format;
...
int hpgv_gl_get_dbformat()
{
    ...
    if (theGLContext->databuf_format == HPGV_RGBA) {
        return HPGV_RGBA;
    } else if (theGLContext->databuf_format == HPGV_RAF) {
        return HPGV_RAF;
    } else {
        HPGV_ABORT("Unsuported format", HPGV_ERROR);        
    }
    
    return theGLContext->databuf_format;
}
...
int hpgv_gl_dbtype(int type)
{
    ...
    if (type != HPGV_FLOAT) {
        char error[1024];
        sprintf(error, "Unsupported type %d", type);
        HPGV_ABORT(error, 1);
    }
    
    theGLContext->databuf_type = type;
...
int hpgv_gl_get_dbtype()
{
    ...
    return theGLContext->databuf_type;
}
...
int hpgv_gl_fragdata(int x, int y, int format, void *data)
{
    ...
    if (format == HPGV_RAF) {  
        ...
        int i = 0;
        for (i = 0; i < HPGV_RAF_ALPHA_BIN_NUM; i++) {
//            printf("offset %d, i %d, value %lf\n", offset, i, in->raf[i]);
...
int hpgv_formatsize(int format)
{
    switch (format) {        
        ...
        case HPGV_RAF : return HPGV_RAF_ALPHA_BIN_NUM * 2;
        case HPGV_RAF_SEG : return HPGV_RAF_SEG_NUM * 2;
 ...
 *
 */
int hpgv_vis_saveraf(int width, int height, int format, int type, int imgnum,
                     const void *pixels, const char *filename)
{
    if (pixels == NULL) {
        fprintf(stderr, "Empty pixel data.\n");
    ...
    HPGV_ASSERT(format == HPGV_RAF, "Unsupported format", HPGV_ERROR);
    HPGV_ASSERT(type == HPGV_FLOAT, "Unsupported type", HPGV_ERROR);
