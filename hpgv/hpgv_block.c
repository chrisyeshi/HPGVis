...
void block_read_data(block_t *block, int vol, char *filename)
{
    ...
    MPI_Get_count(&status, MPI_BYTE, &msgcount);   
 ...
 *
 */
void block_header_new(int id, MPI_Comm mpicomm, int groupsize,
 ...
 *
 */
void block_set_volume(block_t *block, int vol, int datatype, void *data, int varname)
{
    HPGV_ASSERT(block->volume_num > vol && vol >= 0, "Wrong index", HPGV_ERR_MEM);
    HPGV_ASSERT(block != NULL, "Empty data", HPGV_ERR_MEM);

    // free original volume
    volume_finalize(block->volume_data[vol]);
